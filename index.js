...
function random_rgba() {
  var o = Math.round,
    r = Math.random,
    s = 255;
  return "rgba(" + o(r() * s) + "," + 0 + "," + o(r() * s) + "," + "0.03" + ")";
}
function Particle(x, y) {
  this.x = x;
  this.y = y;
  this.vel = {
    x: getRandomFloat(-2, 20) / 100,
    y: getRandomFloat(-2, 20) / 100,
...
Particle.prototype.update = function () {
  const forceDirection = { x: getRandomFloat(-1, 1), y: getRandomFloat(-1, 1) };
...
function init() {
  ...
  for (i = 0; i < numberParticlesStart; i++) {
    ...
    particles.push(
      new Particle(
        windowWidth * (windowWidth > 767 ? 0.67 : 0.5) +
          Math.cos(angle) * circleWidth,
